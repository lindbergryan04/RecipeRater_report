...
Since we are predicting one of three categorical buckets, we decided to use a random forest model. We used the 'minutes', 'n_steps', 'n_ingredients', 'calories', 'sugar', 'carbs', 'sat_fat', 'sodium', and 'protein' features. There are all discrete quantitative features. We used 100 trees per forest and had 10 NOT SURE. We used a test/train split of 80/20, meaning that we trained on 80% of our data and tested the accuracy on the following 20%. Our model had a classification accuracy of 57%, meaning we correctly identified the bucket the recipe fell into at a 57% clip. Much better than randomly assigning them uniformly! Therefore, we feel that this baseline model is good. We might've been able to be more accurate by predicting an average rating of 5 everytime, but this would put us at severe risk of overfitting to our training data. Since the model is based on features from every dataset, we feel it does a good job.

--

## Final Model

When it came time to craft our final, new and improved model, we engineered some new features in hopes of better accuracy. We went a little bit crazy with the feature engineering, and found 17 features that we added to our new model. These weren't all that we created, we were trying to be aware of the risks of adding too many features to cause overfitting, or adding redundant/irrelevant features that would increase our models variance. We chose these 17 because they had the best F scores, F scores being the variance within groups divided by the variance between groups. We then put our fancy pants on and utilized SKLearn's recursive feature elimination to wittle it down to 10. This works by recursively removing the least important feature until we reached our target number. We ended up with our optomized features being: 'log(protein)', 'log(minutes)', 'log(sugar)', 'log(sodium)', 'log(calories)', 'n_steps * n_ingredients', 'n_ingredients * sat_fat', 'n_steps * sat_fat', 'calories', 'sugar'.
